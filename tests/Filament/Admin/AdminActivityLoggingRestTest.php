...
it('logs both the rename and the permission change on a role', function () {
    $role = Role::factory()->create(['name' => 'Old Name', 'guard_name' => 'web']);
    ...
    expect($changes->pluck('name.new')->filter()->all())->toBe(['New Name'])
        ->and($changes->pluck('permissions.new')->filter()->flatten()->all())->toContain(RolePermissionModels::User->viewAny());
});
